    ...
    if mode == "split":
        ...
        for node in body.descendants:
            ...
            if node.name == "span":
                ...
                # This site quotes a source in one span and its translation in
                # the next: <blockquote><span class="en">original</span>
                # <span class="zh">译文</span><cite>…</cite></blockquote>.
                # blockquote is not a narration tag and a span under it fails
                # the div rule below, so without this branch every quote is
                # silent — and nearly every quote here is introduced with a
                # colon, so the listener hears the run-up and then nothing.
                # The English half is dropped by the language check further
                # down, same as anywhere else on the page.
                if pname == "blockquote":
                    pass
                elif pname != "div":
                    continue
                elif not any(getattr(c, "name", None) in _BLOCK_TAGS
